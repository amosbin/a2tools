...
# dms_resolve_dir [DIR]: print the docker-mailserver stack directory (the one
# holding compose.yaml). Tried in order: DIR, $DMS_DIR (env or config), the
# podmgr layout, the historical default. Fails when none exists.
dms_resolve_dir() {
    local d
    for d in "${1:-}" "${DMS_DIR:-}" /srv/podmgr/compose/dms /opt/compose/docker-mailserver; do
        if [ -n "$d" ] && [ -d "$d" ]; then
            printf '%s\n' "$d"
            return 0
        fi
...
# dms_config_dir DMS_DIR: print the directory the container mounts as
# /tmp/docker-mailserver: DMS_DIR/config (plain compose layout) or
# DMS_DIR/data/config (podmgr layout).
dms_config_dir() {
    local d
    for d in "$1/config" "$1/data/config"; do
        if [ -d "$d" ]; then
            printf '%s\n' "$d"
            return 0
        fi
...
# dms_owner DMS_DIR: print the host user owning the stack (compose.yaml).
# With rootless podman that user IS the container's root, so it is the one
# that must be able to read the certificates.
dms_owner() {
    stat -c '%U' "$1/compose.yaml" 2>/dev/null
}

# dms_grant_cert_access USER FQDN: let USER read the Let's Encrypt
# certificate for FQDN (live/ symlinks and the archive/ files behind them).
#
# /etc/letsencrypt is root-only, so a rootless mailserver cannot load its
# certificate without this. Must be re-run after every renewal: certbot
# writes new private keys with mode 0600, whose ACL mask hides any named
# entry, so a default ACL alone does not survive a renewal.
dms_grant_cert_access() {
    local user="$1" fqdn="$2" le=/etc/letsencrypt d
    if ! command -v setfacl >/dev/null 2>&1; then
        log_msg "dms: setfacl not installed; cannot let $user read $fqdn"
        return 1
    fi
    id -u "$user" >/dev/null 2>&1 || return 1
    [ -d "$le/live/$fqdn" ] || return 1
